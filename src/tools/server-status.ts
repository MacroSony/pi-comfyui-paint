  ...
  return {
    ...
    async execute(_params, signal) {
      ...
      const lines = [
        ...
        `Job ID style: ${config.jobIdStyle}`,
        `Background reconciliation: ${config.reconcileIntervalMs === 0 ? "disabled" : `${config.reconcileIntervalMs / 1000}s`}`,
        `Config files: ${config.configFiles.length > 0 ? config.configFiles.join(", ") : "none (env/defaults only)"}`,
      ...
      return {
        ...
        details: {
          ...
          jobIdStyle: config.jobIdStyle,
          backendOutputDirs: config.backendOutputDirs,
